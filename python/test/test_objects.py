...
class NautAngleTest(unittest.TestCase):
    ...
    def test_init_with_float(self):
        a = NautAngle(np.radians(315))
        self.assertAlmostEqual(a.degrees, -45)
        self.assertAlmostEqual(a.real, np.radians(-45))
        self.assertAlmostEqual(a, np.radians(-45))

    def test_init_with_string(self):
        a = NautAngle('-45')
        self.assertAlmostEqual(a.real, np.radians(-45))
        a = NautAngle('45.5s')
        self.assertAlmostEqual(a.real, np.radians(-45.5))
        a = NautAngle('W 45.')
        self.assertAlmostEqual(a.real, np.radians(-45))
        a = NautAngle('N-45')
        self.assertAlmostEqual(a.real, np.radians(-45))
    ...
    def test_init_edge_cases(self):
        a = NautAngle('180')
        self.assertAlmostEqual(a.real, -np.pi)
        a = NautAngle('-180')
        self.assertAlmostEqual(a.real, -np.pi)
        a = NautAngle('0S')
    ...
    def test_string_conversion(self):
        a = NautAngle('-30.3137')
        self.assertEqual(str(a), '-30 18.8220')
        self.assertEqual(repr(a), '-30.3137')

    def test_distance_to(self):
        a = NautAngle('-45')
        b = NautAngle('170')
        self.assertAlmostEqual(float(a.distance_to(b)), np.radians(145))
        self.assertAlmostEqual(float(b.distance_to(a)), np.radians(-145))

    def test_distance_to_edge_cases(self):
        a = NautAngle('90')
        b = NautAngle('-90')
        self.assertEqual(float(a.distance_to(b)), -np.pi)
        self.assertEqual(float(b.distance_to(a)), -np.pi)
